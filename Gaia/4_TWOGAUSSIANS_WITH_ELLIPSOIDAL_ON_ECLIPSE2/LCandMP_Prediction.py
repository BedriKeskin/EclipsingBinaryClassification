...
# --- Sadece TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2 olan source_id'ler ---
allowed_ids = set(df_all.loc[df_all["model_type"] == "TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2", "source_id"])

# --- Kaynak PNG'lerden yalnızca TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2 olanları filtrele ---
...
if not twogauss_pngs:
    raise SystemExit("Uygun (TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2) PNG bulunamadı.")

random.seed(RANDOM_SEED)
selected_files = random.sample(twogauss_pngs, min(SAMPLE_COUNT, len(twogauss_pngs)))
print(f"TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2 PNG sayısı: {len(twogauss_pngs)} | Seçilen: {len(selected_files)}")
...
for img_path in selected_files:
    try:
        ...
        # (Güvence) TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2 mı?
        if str(df.loc[sid, 'model_type']).strip() != 'TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2':
            continue
...
print("===== ÖZET =====")
print(f"Seçilen (TWOGAUSSIANS_WITH_ELLIPSOIDAL_ON_ECLIPSE2) : {len(selected_files)}")
