...
# --- Sadece ONEGAUSSIAN_WITH_ELLIPSOIDAL olan source_id'ler ---
allowed_ids = set(df_all.loc[df_all["model_type"] == "ONEGAUSSIAN_WITH_ELLIPSOIDAL", "source_id"])
# --- Kaynak PNG'lerden yalnızca ONEGAUSSIAN_WITH_ELLIPSOIDAL olanları filtrele ---
...
if not twogauss_pngs:
    raise SystemExit("Uygun (ONEGAUSSIAN_WITH_ELLIPSOIDAL) PNG bulunamadı.")

random.seed(RANDOM_SEED)
selected_files = random.sample(twogauss_pngs, min(SAMPLE_COUNT, len(twogauss_pngs)))
print(f"ONEGAUSSIAN_WITH_ELLIPSOIDAL PNG sayısı: {len(twogauss_pngs)} | Seçilen: {len(selected_files)}")
...
for img_path in selected_files:
    try:
        ...
        # (Güvence) ONEGAUSSIAN_WITH_ELLIPSOIDAL mı?
        if str(df.loc[sid, 'model_type']).strip() != 'ONEGAUSSIAN_WITH_ELLIPSOIDAL':
            continue
...
print("===== ÖZET =====")
print(f"Seçilen (ONEGAUSSIAN_WITH_ELLIPSOIDAL) : {len(selected_files)}")
